...
- 키 탐색 및 검색:
  - 커서 기반 키 스캔(`KV_SCAN`, `PaginationStyle::Cursor`). Cluster 연결에서는 일반 `SCAN`이 단일 노드 기준으로 의미가 없으므로 드라이버는 모든 마스터에 걸쳐 팬아웃합니다: 페이지 예산은 아직 대기 중인 마스터들에게 고르게 분배되고, 각 노드의 커서는 독립적으로 추적되며, 집계된 커서는 `"<host>:<port>"`를 해당 노드의 대기 중인 `SCAN` 커서에 매핑하는 불투명한 JSON 개체로 왕복합니다. 전체 스캔은 모든 마스터가 커서 0을 보고하면 소진됩니다.
