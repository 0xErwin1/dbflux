...
- 명시적 문 실행 시간 제한과 행 수 제한이 있는 인스턴스 지표/검사기 요청은 실행 전에 거부됩니다. 제한 없는 요청은 기존 동작을 유지합니다. 보관 행 수 제한은 바이트 수, 서버 작업량, 엔진 메모리 할당 또는 경과 시간을 제한하지 않습니다. 호스팅된 MariaDB는 검증하지 않았습니다.
