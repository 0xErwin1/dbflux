...
- 실행 결과 제한(바운디드 실행): 요청된 행 제한(`QueryRequest::limit`)은 수집 중에 적용됩니다 — tiberius 스트림을 느리게(lazily) 읽어 요청된 행만 변환하고 보존합니다. 하나의 남은 행 예산이 execute 요청 전체에서 공유됩니다: 배치의 모든 서버 결과 집합이 실제 스트림 순서대로 같은 예산에서 소비하며(제한은 set별 상한이 아니라 총량입니다), 각 집합은 끝까지 드레인되므로 변경 작업의 효과가 온전히 완료되고, 예산이 소진된 후에도 이후 문장이 계속 실행되며, 늦게 도착하는 오류는 버려지지 않고 전파되고, 각 집합은 실제로 행을 생략했을 때만 `rows_truncated`를 보고합니다. 기본 결과는 여전히 마지막 집합입니다. 제한 `0`은 행을 하나도 보존하지 않지만 모든 것을 실행하고 드레인합니다. 제한이 없는 요청은 기존의 무제한 동작을 유지합니다.
- 안전하게 제한할 수 없는 요청은 실행 전에, 보류 중인 취소 신호를 지우지 않고 거부합니다: 요청된 명령문 타임아웃, 그리고 드라이버의 인스턴스 메트릭이나 검사기를 향한 행 제한 요청이 그렇습니다.
...
- 바운디드 실행은 보존 상한이지 서버 측 한도가 아닙니다: 문장은 서버에서 끝까지 실행되고, 상한을 넘는 모든 행은 수신 후 폐기되며, 서버 작업·시간·바이트 예산은 따로 적용되지 않습니다. 상한은 보존되는 행에 적용되고 전송되는 총 바이트에 적용되지 않으며, 하나의 거대한 행은 온전히 보존됩니다. 행 제한 하의 변경 작업은 모든 효과를 완료합니다.
- 행 제한 상한은 일반 SQL 실행에 적용되며 모든 execute 컨텍스트에 적용되는 것이 아닙니다: 드라이버의 인스턴스 메트릭이나 검사기를 향한 제한된 요청은 연결 잠금이나 어떤 디스패치보다 먼저 거부됩니다 — 그런 내부 카탈로그 쿼리에는 상한을 적용할 자리가 없습니다 — 반면 제한 없는 요청은 계속 동작합니다.
- 요청된 명령문 타임아웃(`QueryRequest::statement_timeout`)은 지원되지 않으며 실행 전에 거부됩니다: 드라이버는 자체 데드라인 메커니즘을 구현하지 않습니다 — 워치독도, `KILL` 기반 데드라인도, 세션 수준 `SET`도 없습니다. 제한 없는 일반 쿼리는 기존 `KILL` 경로로 계속 취소할 수 있습니다.
- 바운디드 실행 및 거부 동작은 `mcr.microsoft.com/mssql/server:2022-latest` 컨테이너 이미지(SQL Server 2022)로만 라이브 테스트되었습니다. Azure SQL Database와 Managed Instance에서는 테스트하지 않았습니다.
