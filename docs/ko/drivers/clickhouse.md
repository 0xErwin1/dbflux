...
- 페이지 나누기, 정렬, 필터링은 드라이버가 문 주위에 `LIMIT`/`OFFSET`를 붙여 적용하며, 이 덕분에 커서 없이도 결과 탐색이 동작합니다.
...
- SSH 터널을 지원하지 않습니다.
- 트랜잭션, 준비된 문(prepared statement), 쿼리 취소를 지원하지 않습니다. 실행 중인 문은 요청 시간 초과 외에는 제한되지 않으며, 드라이버는 잠금 시간 초과 지원을 보고하지 않습니다.
