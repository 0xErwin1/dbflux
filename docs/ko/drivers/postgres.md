...
- `infinity`와 `-infinity` 날짜와 타임스탬프, 그리고 그 1차원 배열을 PostgreSQL 자체의 텍스트로 표시합니다.
...
- 행 제한은 결과를 스트리밍하면서 요청 전체에 걸쳐 요청한 수의 행만 보관하고, 결과 집합마다 생략된 행이 있는지 표시합니다. 실행은 항상 끝까지 완료됩니다.
- 행 제한이 있는 다중 문 배치는 같은 타입 지정 스트리밍 경로로 문을 하나씩 실행하며, 요청 전체가 하나의 행 예산을 공유합니다. 예산이 소진된 뒤의 문도 계속 실행되고, 결과 집합은 제한 없는 실행과 같은 순서를 유지하며(첫 번째가 기본 결과), 배치는 첫 번째 실패에서 멈춥니다.
- 행 제한이 있는 배치는 같은 스크립트를 제한 없이 하나의 단순 쿼리로 실행할 때와 같은 트랜잭션 경계를 유지합니다. 사용자 트랜잭션 밖의 문은 드라이버가 열고 커밋하는 트랜잭션 안에서 실행되며, 실패하면 롤백되므로 어떤 문도 남지 않습니다. 스크립트의 `BEGIN`은 앞선 문을 그 트랜잭션에 포함하고 `COMMIT` 또는 `ROLLBACK`은 이를 끝냅니다. 이는 PostgreSQL의 암시적 트랜잭션 블록과 같습니다. 열려 있는 세션 트랜잭션 안에서는 문이 그 트랜잭션에 합류하며, 실패하면 트랜잭션이 중단된 상태로 남습니다.
- 요청된 실행 시간 제한은 실행 전에 거부됩니다.

## 제한 사항

- 행 제한은 서버 작업, 네트워크 전송량, 실행 시간이 아닌 보관량만 제한합니다. 변경 작업은 모든 효과를 완료합니다.
- 인스턴스 지표와 검사기의 행 제한은 실행 전에 거부됩니다. 제한 없는 배치는 기존 동작을 유지합니다.
- 행 제한이 있는 배치는 어떤 문도 실행하기 전에 `PREPARE TRANSACTION`과, 명시적 트랜잭션 밖의 문 뒤에 오는 `SAVEPOINT`, `RELEASE`, `ROLLBACK TO` 또는 체인된 `COMMIT`/`ROLLBACK`을 거부합니다. PostgreSQL은 암시적 트랜잭션 블록 안에서 이들을 거부하지만, 드라이버가 여는 트랜잭션은 이들을 허용하기 때문입니다.
- `VACUUM`이나 `CREATE INDEX CONCURRENTLY`처럼 트랜잭션 블록 안에서 실행할 수 없는 문은 행 제한이 있는 배치 안에서 PostgreSQL 자체 오류로 실패하며, 이는 제한 없는 배치와 같습니다.
- 행 제한이 있는 배치는 SQL 편집기의 문 분할기로 나뉘는데, 이 분할기는 SQL 표준 `BEGIN ATOMIC` 함수 본문을 인식하지 않습니다. 이를 포함한 배치는 실행되지 않고 구문 오류로 실패하며 롤백됩니다. 같은 함수만 담은 요청은 정상적으로 실행됩니다.

- 제한 없는 배치(다중 문) 결과 열에는 타입 메타데이터가 없습니다. 값은 텍스트로 반환되며 이에 대해서는 차트 자동 감지가 비활성화됩니다. 완전한 타입 정보를 가진 열이 필요하면 단일 문을 실행하세요.
...
- 드라이버가 디코딩할 수 없는 단일 문의 `NULL`이 아닌 값(예: 디코더가 없는 타입의 값이나 표현할 수 있는 범위를 벗어난 날짜)은 지원되지 않는 타입으로 표시되고 결과에 표시가 남습니다. 실제 `NULL`은 계속 `NULL`로 표시됩니다. 서버 자체의 텍스트를 읽으려면 열을 `text`로 캐스팅하세요. 인스턴스 검사기는 그런 셀의 열과 타입을 로그에 기록합니다.
