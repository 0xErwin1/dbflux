...
- 键浏览与发现：
  - 基于游标的键扫描（`KV_SCAN`、`PaginationStyle::Cursor`）。在 Cluster 连接上，单纯的 `SCAN` 没有单节点含义，因此驱动程序会把它扇出到每个主节点：页预算在仍有待处理的主节点之间均分，每个节点的游标独立跟踪，聚合后的游标以不透明的 JSON 对象往返，把 `"<host>:<port>"` 映射到其待处理的 `SCAN` 游标。当所有主节点都报告游标 0 时，整体扫描才算结束。
