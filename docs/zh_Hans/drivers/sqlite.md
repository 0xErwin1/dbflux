# SQLite

嵌入式、基于文件的数据库。

## 速览

- **类别** —— 关系型
- **查询语言** —— SQL
- **URI 方案** —— `sqlite`

## 功能

- 嵌入式 SQLite 关系型驱动程序，使用基于文件的数据库路径。
- 支持 SQL 执行、Schema 发现、视图、索引、外键、CHECK 约束与唯一约束。
- 通过 SQLite 的中断句柄支持查询取消，并由一个进度处理器在语句运行期间检查取消请求，因此在第一条语句开始之前到达的取消仍会结束查询。
- 包含面向 CRUD、索引、重建索引（reindex）、创建表与删除表的 SQL/代码生成。
- 多语句脚本（若干以 `;` 分隔的语句）会被切分并逐条执行，每条都走预编译路径，每个语句返回一个结果集。（`rusqlite::prepare` 只会解析字符串中的第一条语句，因此脚本必须先切分。）
- 对每条产生行的单条语句（`SELECT`、`PRAGMA`、`EXPLAIN`、`WITH ... SELECT`、`VALUES` 以及带 `RETURNING` 的 DML）强制执行请求的行数上限：迭代过程中只保留请求的行，迭代会一直抽干到结束，因此变更（mutation）的效果总是完整落盘，迟到的逐行错误仍然会传播，并且只有确实省略了行时结果才报告截断。
- 带行数上限的多语句批次会逐条执行，整个请求共享同一个行数预算。批次使用 SQLite 自身的词法分析器切分，因此反斜杠不是转义符，字面量、注释和触发器主体中的分号不会切分语句；末尾的注释或分号不会产生额外的语句。每条产生行的语句只在预算尚有剩余时保留行，预算耗尽之后的语句仍会执行，并且批次与不带上限的批次完全一样，在第一个失败处停止。
- 在任何预编译或执行之前拒绝无法安全设定边界的请求：指向实例指标（metrics）或检查器（inspectors）的带行数上限请求，以及请求的语句超时。
- 数据传输引擎：原生的多行 `INSERT` 批量装载（`BULK_INSERT`）、依据源表列生成的驱动程序原生 `CREATE TABLE` DDL，以及用于外键安全迁移的按连接参照完整性开关（`PRAGMA foreign_keys`）。

## 限制

- 仅支持本地文件；没有网络传输、SSH 隧道，也没有 TLS/SSL 模式。
- 请求的行数上限是保留上限，不是引擎侧的限额：语句仍会在内嵌引擎中完整执行，超出上限的所有行都会被观察后丢弃，并且不施加任何字节、内存或时间预算 —— sorter 与 `RETURNING` 的内存分配不受上限约束。带行数上限的变更会完成其全部效果。
- 切分带行数上限的批次不会预编译任何内容，但会在每个 `;` 处用 SQLite 的词法分析器检查语句文本，对于字面量、注释或触发器主体中含有大量分号的语句，其开销与语句长度呈二次方关系。不带上限的批次保留原有的切分执行行为。
- 不带行数上限时，`WITH ... SELECT`、`VALUES` 以及带 `RETURNING` 的 DML 保留原有行为：语句已经执行之后才报告不支持的执行错误。
- 指向驱动程序实例指标或检查器的带行数上限请求会在连接锁或任何 dispatch 之前被拒绝 —— 尽管驱动程序未声明实例目录，公开的请求类型仍可能携带这些上下文 —— 而不带上限的请求保持现有行为。
- 请求的语句超时（`QueryRequest::statement_timeout`）不受支持，会在执行前被拒绝。不带上限的普通查询仍可通过现有的中断路径取消。
- 仅支持 SQL 的驱动程序；不提供文档型或键值 API。
- SQLite 的 Schema 模型没有服务端的多 schema 命名空间与之对应。
- 没有 `TRUNCATE TABLE` 语句；数据传输引擎的 Truncate 装载选项对 SQLite 目标不可用（未设置 `DriverCapabilities::TRUNCATE_TABLE`）。

## DDL 能力

### 事务性 DDL

SQLite 支持**事务性 DDL** —— 手动 DDL 操作可以包在事务中并回滚：

```sql
BEGIN;
ALTER TABLE users ADD COLUMN phone TEXT NULL;
-- 验证这项改动
ROLLBACK;  -- 出现问题时可以安全回滚
```

托管表更改有所不同：其规划器要求使用自动提交连接，因为驱动程序负责每张表的事务。

### 托管 ALTER TABLE

对于纯 `DROP COLUMN` 请求，当链接的 SQLite 版本支持时，DBFlux 会优先使用 SQLite 原生的 `DROP COLUMN` 路径。原生删除保留 SQLite 自身的接受规则，不受重建语法限制。DBFlux 仍会预检已知依赖项，SQLite 也会在执行时再次验证请求。

对于选定的类型、可空性或默认值变更，以及当链接的 SQLite 版本没有原生 `DROP COLUMN` 时的纯删除，DBFlux 会保守地重建 `main` schema 中的一张表。重建请求也可以将这些列变更与删除操作组合。

重建会复制保留的行，不进行强制类型转换、数据转换或回填。它会精确保留已验证的行标识和存储值；变更后的默认值仅适用于未来插入。因此，当现有行不兼容时，将列改为必填可能失败。

准备过程是只读的。通用 UI 和 MCP 中展示的预览是不可变、说明性的生命周期描述，不是可执行的应用 SQL。驱动程序负责私有替换表、复制和精确比较、源表替换、已验证显式索引的恢复以及最终验证。

每次重建仅对一张表具有原子性。发生故障时，驱动程序会验证回滚和连接设置的恢复。清理失败会保持可见；仅在事务状态不确定或无法恢复连接设置时，连接才会被隔离，直到重新连接。不存在跨表原子性。

#### 重建范围

重建路径仅接受能够证明在 `main` schema 中安全的表形态。例如，它会拒绝 CHECK 约束、生成列、`AUTOINCREMENT`、`STRICT`、`WITHOUT ROWID`、视图或触发器、表达式或部分索引、附加 schema，以及调用方拥有的活动事务。这些限制仅适用于重建：适用的原生纯删除不会仅因超出重建语法而被拒绝。

### 索引操作

**CREATE INDEX**：
- 在此期间锁住数据库（阻塞写入）
- 没有并发选项（与 PostgreSQL 不同）

**DROP INDEX**：
- 快（仅涉及元数据）

**REINDEX**：
- 重建索引（锁住数据库）

### 约束

**添加约束**：
- SQLite 在 `INSERT`/`UPDATE` 时校验约束
- 无法为已有的表添加约束（需要重建表）

**外键**：
- 默认关闭（必须用 `PRAGMA foreign_keys = ON` 启用）
- 无法添加到已有的表（需要重建表）

### 已知限制

- 托管表更改规划器会变更选定的类型、可空性、默认值和删除操作；它不会向现有表添加约束。
- 重建会严格限制为上文所述已验证的表形态。
- 不能并发创建索引（会锁住数据库）。
- 动态类型（列类型只是建议性的）。

### 最佳实践

1. **对手动 DDL 使用事务** —— 托管表更改要求自动提交。
2. **提前规划 Schema** —— 事后修改很困难。
3. **检查托管 ALTER 预览** —— 它仅供说明；应通过 DBFlux 应用，而不是执行其中的语句。
4. **单独规划不支持的表形态** —— 重建路径会拒绝视图、触发器和其他未经验证的形态。
5. **先在副本上测试** —— 尤其是在托管重建之前。
6. **启用外键** —— 修改 Schema 之前先执行 `PRAGMA foreign_keys = ON`。
7. **使用 VACUUM** —— 在 `DROP TABLE` 或重建表之后回收磁盘空间。
