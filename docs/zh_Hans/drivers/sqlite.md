...
- 在任何预编译或执行之前拒绝无法安全设定边界的请求：行数上限与多语句批次组合的请求（末尾的注释或分号不会构成批次）、指向实例指标（metrics）或检查器（inspectors）的带行数上限请求，以及请求的语句超时。
...
- 带行数上限的请求无法执行多语句批次：准备或执行批次可能在任何上限生效之前就执行了前面的语句，因此这类请求会在准备任何语句之前被拒绝。不带上限的批次保留原有的切分执行行为。末尾仅有注释或分号的带上限单条语句不会被当作批次。
