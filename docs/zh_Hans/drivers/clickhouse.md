...
- SQL 不经改写直接通过 HTTP 发送。现有的 `offset` 请求值作为 HTTP URL 参数发送；此处不保证服务器如何处理该参数。
...
- 不支持 SSH 隧道。
- 不支持事务、预编译语句与查询取消。显式指定的 `QueryRequest` 行数限制（包括零）和语句超时会在派发前被拒绝。已配置的默认 HTTP 传输超时仍然生效，但不能保证服务器上的工作或数据变更停止。无限制的查询可能在内存中分配完整结果（受 HTTP 响应体上限约束）。该驱动程序也不声明支持锁超时。
