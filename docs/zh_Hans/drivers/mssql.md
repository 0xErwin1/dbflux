...
- 在创建元数据支持（DBF-161 PR1）之前采集的深度快照不携带元数据；将其用作 diff 引用会拒绝创建，必须重新采集快照。UI 连接时的自动采集目前是**浅层**的，因此**保存的**快照只有在通过深度快照 API 以编程方式采集时才携带创建元数据；当 diff 引用是**活动连接**时，元数据通过通用的 `table_creation_metadata` 接缝获取，无需任何存储快照即可生成。
