...
| `StackedBar` | 堆叠柱状图。每个 X 位置为每个系列显示一根柱子，按累加方式堆叠，而非并排分组。各系列按 X 值对齐：某个系列在某个 X 上没有值时，不会为该柱子增加任何部分。Y 轴在渲染时重新缩放为从零到最大堆叠总和，刻度标签也使用同一范围。 |
...
在类别为 `DatabaseCategory::TimeSeries` 的连接（例如 InfluxDB 的 measurement）上打开集合时，其数据网格会提供与查询结果相同的 Data、Chart 和 JSON 视图。当 `detect_chart_columns` 返回 `Ok` 时，第一页以图表形式打开，坐标轴由 `default_bindings_for_time_series` 预设（X 轴为时间，Y 轴为第一个数值列，分组为第一个 `Text` 列）。分组会为其列的每个不同值绘制一条线，并以该值作为标签，因此像 `host` 这样的标签会让每台主机拥有自己的线。在坐标轴栏中选择分组时同样如此。只有当该列在结果中的不同值不超过 12 个（`DEFAULT_GROUP_MAX_VALUES`）时才会应用默认分组，否则图表以不分组状态打开。在坐标轴栏中手动选择的分组没有此限制。Data 视图以网格显示各行，而不是其他集合使用的文档树。刷新（无论是手动、自动还是翻页）会保留用户选择的视图，只有当新的一页无法再绘制图表时才回退到 Data。
