...
当你的某项操作失败时，DBFlux 会弹出一个带**在审计中查看**操作的 Toast 提示。点击它会打开审计查看器，并筛选出该条事件（依据是 Toast 提示与审计行之间共享的关联 ID）。状态栏上的**错误徽标**则会打开已预先筛选为近期面向用户失败的查看器。

### 导出

**导出**按钮会把当前可见的事件以 **CSV** 或 **JSON** 写出，采用扩展结构（包含全部字段，含结构化详情）。保存对话框会询问写入位置，并建议一个带时间戳的文件名（`audit_export_20260923-140507.csv`），因此第二次导出不会覆盖第一次的文件。取消对话框则不写入任何内容。当原生文件选择器不可用时，文件会像其他导出一样写入 `~/.local/share/dbflux/exports/`。成功后会弹出 Toast 提示，说明写入了多少条事件以及存放位置。
