...
`Connection`](../crates/dbflux_core/src/core/traits.rs):
...
`DriverCapabilities`](../crates/dbflux_core/src/driver/capabilities.rs),
...
en `Connection` en [`traits.rs`](../crates/dbflux_core/src/core/traits.rs).
...
`ColumnKind`](../crates/dbflux_core/src/query/types.rs). Los charts y otros
...
- [ ] Añade `crates/dbflux_driver_<name>/Cargo.toml`, `src/lib.rs`, los módulos
...
- [ ] Implementa formateo estructurado de errores de query y conexión mediante
  ...
  `ConnectionErrorFormatter`](../crates/dbflux_core/src/core/error_formatter.rs).
...
- [ ] Configura el servicio bajo Settings -> RPC Services como `kind=driver` con
  un `socket_id` estable; añade un comando gestionado solo cuando DBFlux deba
