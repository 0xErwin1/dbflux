...
Driver de Amazon Redshift para DBFlux (v1 de solo lectura), construido
...
- Los valores de OID de tipo extendido usados para
  ...
  (`crates/dbflux_driver_redshift/tests/live_integration.rs`), ya que no existe
  ningún motor de Redshift local ni basado en Docker. Ejecútalas explícitamente
  contra un cluster real con `cargo nextest run -p dbflux_driver_redshift
  --run-ignored all`.
