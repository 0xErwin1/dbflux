...
impl SchemaGraph {
    ...
    pub fn neighborhood(&self, table: &str, schema: Option<&str>, depth: usize) -> SchemaGraph {
        ...
        for edge in self.graph.edge_indices() {
            ...
            let Some(&new_source) = new_index_by_old.get(&source) else {
                continue;
            };
            let Some(&new_target) = new_index_by_old.get(&target) else {
                continue;
            };
        ...
        let node_index_by_id = new_index_by_old
            .iter()
            .filter_map(|(_old_idx, &new_idx)| {
                subgraph
                    .node_weight(new_idx)
                    .map(|node| (node.id.clone(), new_idx))
            })
            .collect();
    ...
    pub(crate) fn focal_subgraph(&self, focal_id: &TableNodeId) -> SchemaGraph {
        ...
        for edge_idx in self.graph.edge_indices() {
            ...
            let Some(weight) = self.graph.edge_weight(edge_idx) else {
                continue;
            };
            let Some(&new_source) = new_index_by_old.get(&source) else {
                continue;
