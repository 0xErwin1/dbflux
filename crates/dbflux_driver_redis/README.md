...
- Key browsing and discovery:
  - Cursor-based key scanning (`KV_SCAN`, `PaginationStyle::Cursor`). One page request keeps issuing `SCAN` until the page holds the requested number of keys or the scan completes, so a sparse `MATCH` filter does not produce empty pages. Each page is bounded to 1000 `SCAN` round trips and 500 ms; when the bound is reached, the page returns the keys found so far with a pending cursor. A key that `SCAN` repeats is listed once per page, and a page can exceed the requested size by at most one `SCAN` batch. The page's key types are fetched in one pipeline. On a Cluster connection, a plain `SCAN` has no single-node meaning, so the driver scans the pending masters in turn under the same page bound: each node's cursor is tracked independently, and the aggregated cursor round-trips as an opaque JSON object mapping `"<host>:<port>"` to its pending `SCAN` cursor. The overall scan is exhausted once every master reports cursor 0.
  - Keyspace key total (`key_count`): `DBSIZE` on the selected database; on a Cluster connection, the sum of `DBSIZE` over every master. The key browser shows it next to the page count when no filter is set.
