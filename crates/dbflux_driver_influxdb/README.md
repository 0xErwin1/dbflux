...
- **Measurement browse opens as a chart** — opening a measurement from the sidebar runs `SELECT * FROM "<measurement>" ORDER BY time DESC LIMIT <n> OFFSET <m>` over InfluxQL (on v2 through the InfluxQL compatibility endpoint) against the bucket or database the measurement belongs to. `InfluxQueryGenerator::collection_browse_query` returns that same statement, so the data grid's toolbar and the status bar show the query that runs, in InfluxQL. The result carries a timestamp column and typed field columns, so the measurement opens in the Chart view, with the rows one click away in the Data view.
...
- **Measurement browse ignores the filter input** — `browse_collection` and `count_collection` do not read the collection filter, so text typed in the data grid's filter box does not narrow the rows.
- **Measurement browse has no time window** — the browse reads the newest rows across the whole retention period, and the chart's time-range presets are not offered for a measurement. Use "Query Measurement" for a time-bounded query.
- **Tags and fields are not told apart in a result** — both arrive as ordinary columns. The chart groups by the first text column, which is usually a tag but can be a string field.
