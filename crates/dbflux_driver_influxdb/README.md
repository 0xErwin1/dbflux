...
- **Instance metrics and inspectors are v2-only** — `instance_catalog()` returns `None` on v1 connections. v1 does not expose `/metrics` or `/health` in a form this catalog can rely on, mirroring the v1 write-privilege probe limitation above.
