...
- **No instance metrics or instance inspector yet** (`INSTANCE_METRICS`/`INSTANCE_INSPECTOR` are not declared). InfluxDB's internal monitoring bucket and `/metrics` endpoint are natural candidates for a future `InstanceCatalog`, so this is planned rather than permanently excluded.
