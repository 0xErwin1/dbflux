use crate::{ConnectionProfile, DbError, SshTunnelProfile};
use std::fs;
...
impl SshTunnelStore {
    pub fn new() -> Result<Self, DbError> {
        ...
        Ok(Self {
            path: app_dir.join("ssh_tunnels.json"),
        })
    }

    pub fn load(&self) -> Result<Vec<SshTunnelProfile>, DbError> {
        if !self.path.exists() {
            return Ok(Vec::new());
        }

        let content = fs::read_to_string(&self.path).map_err(DbError::IoError)?;
        let tunnels: Vec<SshTunnelProfile> =
            serde_json::from_str(&content).map_err(|e| DbError::InvalidProfile(e.to_string()))?;

        Ok(tunnels)
    }

    pub fn save(&self, tunnels: &[SshTunnelProfile]) -> Result<(), DbError> {
