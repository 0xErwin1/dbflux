...
- `NUMERIC`/`DECIMAL` values are decoded on a best-effort text basis rather than through a typed binary decoder. The wire protocol returns `NUMERIC` in binary format, so on a live cluster these values may currently surface as `Value::Unsupported` rather than a precise `Value::Decimal`; full precise binary `NUMERIC` decoding is a planned follow-up.
